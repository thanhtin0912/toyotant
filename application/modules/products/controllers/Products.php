...
class Products extends MX_Controller {
	...
	public function admincp_update($id=0){
		...
		$this->load->model('products_catalog/products_catalog_model');
		if($id==0){
		...
		$data = array(
			'result'=> $result[0],
			'cates'=> $this->products_catalog_model->getData(),
			'bodytype'=> $this->categories_model->getDataCommonCode('BODYTYPE'),
			'module'=> $this->module,
			'id'=> $id
		);
	...
	public function admincp_save(){
		...
		if($_POST){
			...
			if($_FILES){
				foreach($fileName as $k=>$v){
					if(isset($_FILES['fileAdmincp']['error'][$k]) && $_FILES['fileAdmincp']['error'][$k]!=4){
						...
						} else if ($typeFileImage == 'video'){
							$tmp_name[$k] = $_FILES['fileAdmincp']["tmp_name"][$k];
							$file_name[$k] = $_FILES['fileAdmincp']['name'][$k];
							$ext = strtolower(substr($file_name[$k], -4, 4));
							$fileName[$k] = date('Y').'/'.date('m').'/'.'video/'.md5(time().'_'.SEO(substr($file_name[$k],0,-5))).$ext;

						} else {
							print 'error-video-upload.'.$this->security->get_csrf_hash();
							exit;
						}
					}
				}
			...
			if($this->model->saveManagement($imagesName,$fileName)){
				//Upload Image
				if($_FILES){
					$upload_path = BASEFOLDER.DIR_UPLOAD_CAR;
	...
	public function detail($slug){
		...
		$this->template->write('meta_image', PATH_URL . DIR_UPLOAD_CAR . $data["item"][0]->image);
