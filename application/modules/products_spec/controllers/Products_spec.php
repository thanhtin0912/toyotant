...
class Products_spec extends MX_Controller {
	...
	public function admincp_update($id=0){
		...
		if($id!=0){
			...
			$listsSpec = $this->model->getListDataSpec($result[0]->id);
		}
		$data = array(
			'cates'=> $this->categories_model->getDataCommonCode('TSKTCAR'),
			'listsSpec' => $listsSpec,
			'result'=>$result[0],
			'module'=>$this->module,
			'id'=>$id
	...
	public function admincp_saveSpec() {
		if($this->model->saveManagement()){
			print 'success.'.$this->security->get_csrf_hash();
			exit;
		}	
	...
	public function admincp_deleteSpec() {
		$this->db->where('id',$this->input->post('id'));
