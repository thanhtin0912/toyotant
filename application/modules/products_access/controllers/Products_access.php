...
class products_access extends MX_Controller {

	private $module = 'products_access';
	private $table = 'sle_access';
	...
	public function admincp_update($id=0){
		...
		$data = array(
			'products'=> $this->products_catalog_model->getData(),
			'cates'=> $this->categories_model->getDataCommonCode('ACCESSCAR'),
			'result'=>$result[0],
			'module'=>$this->module,
			'id'=>$id
		);
	...
	public function admincp_save(){
		...
		if($_POST){
			...
			if($this->model->saveManagement($fileName)){
				...
				if($_FILES){
					if($_FILES){
						$upload_path = BASEFOLDER.DIR_UPLOAD_CAR_ACCESS;
			...
			foreach ($result as $key => $value) {
				if($value->delete == 0){
					...
					if($this->db->delete(PREFIX.$this->table)){
						@unlink(BASEFOLDER.DIR_UPLOAD_CAR_ACCESS.$result[0]->avatar);
						print $this->security->get_csrf_hash();
						exit;
					}
				}
