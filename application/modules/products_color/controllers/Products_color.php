...
class Products_color extends MX_Controller {

	private $module = 'products_color';
	private $table = 'sle_color';
	...
	public function admincp_save(){
		...
		if($_POST){
			...
			if($this->model->saveManagement($fileName)){
				...
				if($_FILES){
					if($_FILES){
						$upload_path = BASEFOLDER.DIR_UPLOAD_CAR_COLOR;
			...
			foreach ($result as $key => $value) {
				if($value->delete == 0){
					...
					if($this->db->delete(PREFIX.$this->table)){
						@unlink(BASEFOLDER.DIR_UPLOAD_CAR_COLOR.$result[0]->avatar);
						print $this->security->get_csrf_hash();
						exit;
					}
				}
