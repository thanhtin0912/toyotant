...
class Products_info extends MX_Controller {

	private $module = 'Products_info';
	private $table = 'sle_info';
	...
	public function admincp_update($id=0){
		...
		$data = array(
			...
			'cates'=> $this->categories_model->getDataCommonCode('INFOCAR'),
			'result'=>$result[0],
			'module'=>$this->module,
			'id'=>$id
		);
	...
	public function admincp_save(){
		...
		if($_POST){
			...
			if($this->model->saveManagement($fileName)){
				...
				if($_FILES){
					if($_FILES){
						$upload_path = BASEFOLDER.DIR_UPLOAD_CAR_INFO;
			...
			foreach ($result as $key => $value) {
				if($value->delete == 0){
					...
					if($this->db->delete(PREFIX.$this->table)){
						@unlink(BASEFOLDER.DIR_UPLOAD_CAR_INFO.$result[0]->avatar);
						print $this->security->get_csrf_hash();
						exit;
					}
				}
